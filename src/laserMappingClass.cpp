...
void LaserMappingClass::init(double map_resolution){
	...
	noise_filter.setRadiusSearch(map_resolution*10);
	noise_filter.setMinNeighborsInRadius (30);
}
...
void LaserMappingClass::updateCurrentPointsToMap(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& pc_in, const Eigen::Isometry3d& pose_current){
	...
	pcl::transformPointCloud(*down_sized_pc, *transformed_pc, pose_current.cast<float>());
	...
	for(int i=currentPosIdX-LASER_CELL_RANGE_HORIZONTAL;i<currentPosIdX+LASER_CELL_RANGE_HORIZONTAL+1;i++){
		for(int j=currentPosIdY-LASER_CELL_RANGE_HORIZONTAL;j<currentPosIdY+LASER_CELL_RANGE_HORIZONTAL+1;j++){
			for(int k=currentPosIdZ-LASER_CELL_RANGE_VERTICAL;k<currentPosIdZ+LASER_CELL_RANGE_VERTICAL+1;k++){
				if(map[i][j][k]!=NULL){
					if(map[i][j][k]->points.size()>0){
						// noise_filter.setInputCloud (map[i][j][k]);
						// noise_filter.filter (*(map[i][j][k]));
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr LaserMappingClass::getMap(void){
    ...
    // ROS_WARN("total points array %d", laserCloudMap->points.size());
