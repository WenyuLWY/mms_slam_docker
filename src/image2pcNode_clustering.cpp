...
ros::Publisher pubVisualPointCloud;
...
void image2pc(){
    ...
    while(1){
        if(!pointCloudBuf.empty() && !colorImageBuf.empty()){
            ...
            if(pointCloudBuf.front()->header.stamp.toSec() - 0.02 > colorImageBuf.front()->header.stamp.toSec()){
            ...
            if(colorImageBuf.front()->header.stamp.toSec() - 0.02 > pointCloudBuf.front()->header.stamp.toSec()){
            ...
            // if( (total_frame>20&&total_frame<40)||(total_frame>127&&total_frame<145) || (total_frame>217&&total_frame<230) || (total_frame>280&&total_frame<315) || (total_frame>394)){//50
            //     continue;
            // }
            ...
            bool is_visual_final[pointcloud_in->points.size()];
            for(int i=0;i<pointcloud_in->points.size();i++){
                is_dynamic_final[i]=false;
                is_visual_final[i]=false;
                ...
                if(main_dynamic_pc->points.size()>10){
                    ...
                    for (int cluster_index=0;cluster_index<cluster_indices.size();cluster_index++){
                        ...
                        while(points_dilated_count < segment_dilated.size()){
                            ...
                            if(points_dilated_count>1800 + ((int)(original_pc_size/2))) {
            ...
            pcl::PointCloud<pcl::PointXYZRGB>::Ptr visual_pc_final(new pcl::PointCloud<pcl::PointXYZRGB>());
            for(int i=0;i<pointcloud_in->points.size();i++){
                if(is_visual_final[i] == true)
                    dynamic_pc_final->push_back(pointcloud_in->points[i]);
                else
                    static_pc_final->push_back(pointcloud_in->points[i]);
                /*
                if(is_dynamic_final[i]==true && is_visual_final[i] == true)
                    dynamic_pc_final->push_back(pointcloud_in->points[i]);
                else if(is_dynamic_final[i]==true && is_visual_final[i] == false){
                    visual_pc_final->push_back(pointcloud_in->points[i]);
                    dynamic_pc_final->push_back(pointcloud_in->points[i]);
                    static_pc_final->push_back(pointcloud_in->points[i]);
                }else
                    static_pc_final->push_back(pointcloud_in->points[i]);
            ...
            pcl::toROSMsg(*static_pc_final, static_pc_msg);
            //pcl::toROSMsg(*static_pc+*dynamic_pc, static_pc_msg);
            ...
            pcl::toROSMsg(*dynamic_pc_final /* + *bounding_box_pc*/, dynamic_pc_msg);
            ...
            //dynamic pc
            sensor_msgs::PointCloud2 visual_pc_msg;
            pcl::toROSMsg(*visual_pc_final /* + *bounding_box_pc*/, visual_pc_msg);
            visual_pc_msg.header.stamp = pointcloud_time;
            visual_pc_msg.header.frame_id = "/camera_depth_optical_frame";
            pubVisualPointCloud.publish(visual_pc_msg);
...
int main(int argc, char **argv)
{
    ...
    pubVisualPointCloud = nh.advertise<sensor_msgs::PointCloud2>("velodyne_points_visual", 100);
